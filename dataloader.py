...
def load_step(save_num):
    with open(f"./ds/MGN/cylinder_dataset/save_{save_num}.pkl", 'rb') as f:
        rollout_data = pickle.load(f)  # pickle.load(fp)
    # ['faces', 'mesh_pos', 'gt_velocity', 'pred_velocity']
    return rollout_data


def get_step(rollout_data, step_num, interp_type='linear'):
    """
    ...
    Vx = rollout_data['velocity'][step_num][:, 0]
    Vy = rollout_data['velocity'][step_num][:, 1]
    P = rollout_data['pressure'][step_num][:, 0]
...
def patch(states, patch_size: tuple, stride: tuple):
    """
    Returns a list of patches of size patch_size from the states
    """
    C = states.shape[0]
    states = torch.from_numpy(states).unsqueeze(0)
    unfolder = torch.nn.Unfold(kernel_size=patch_size, stride=stride, padding=(0, 0))
...
def ds_get(save_num, step_num, patch_size=(64, 64), stride=(32, 32)):
    ...
    patches = patch(to_patch, patch_size=patch_size, stride=stride)[0]
...
if __name__ == '__main__':
    state, mask = ds_get(0, 10, patch_size=(64, 64), stride=(32, 32))
    print(state.shape, mask.shape)
