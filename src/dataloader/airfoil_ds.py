...
class AirfoilDataset(Dataset):
    ...
    def __init__(self, load_dir, resolution: int, patch_size: tuple, stride: tuple, seq_len: int, seq_interval=1,
        ...
        self.N_x_patch, self.N_y_patch = num_patches(x_px, patch_size[0], stride[0]) - 2, num_patches(y_px, patch_size[1], stride[1]) - 2
    ...
    def _patch(self, states: torch.Tensor):
        ...
        # Get rid of outer patches
        states = states[:, :, ph:-ph, pw:-pw]
...
def plot_all_patches():
    ...
    for batch in ds:
        ...
        c_print(f'{state.shape = }, {diffs.shape = }. {pos_id.shape = }', color='green')
        # exit(4)
        break
