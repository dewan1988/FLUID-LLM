...
class EncoderDecoder:
    ...
    def __init__(self, img_dim: tuple, enc_dim):
...
class FFTEncDec(EncoderDecoder):
    def __init__(self, img_dim, enc_dim):
        super().__init__(img_dim, enc_dim)

        self.fft_mask = self.create_fft_mask()

    def create_fft_mask(self):
...
def main():
    ...
    encoder = FFTEncDec(img_dim=patch_size, enc_dim=None)

    state, diffs, bc_mask = dl.ds_get(load_no, step_num)
    state_enc, state_fft = encoder.encode(state, bc_mask)
